...
export default function Navbar() {
  ...
  const navClasses = `fixed top-0 left-0 right-0 z-50 transition-all ${
    scrolled ? 'backdrop-blur bg-black/60 ring-1 ring-white/10' : 'bg-transparent'
  }`
  ...
  const Item = ({ href, children }) => (
    <a
      ...
      className="block px-4 py-2 rounded-md text-sm font-medium text-slate-200 hover:text-white hover:bg-white/5 md:hover:bg-transparent"
    >
      {children}
    </a>
  )
